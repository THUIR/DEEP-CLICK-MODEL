...
def build_shared_nums(shape, num, name):
    """ Builds a theano shared variable filled with a zeros numpy array """
    return theano.shared(value=numpy.zeros(shape, dtype=theano.config.floatX) + num,
                         name=name, borrow=True)
...
class LinearLayer(Layer):
    ...
    def __repr__(self):
        return "{}: W_shape={} b_shape={} activation={}".format(self.__class__.__name__, self.W.shape.eval(),
                                                                self.b.shape.eval(), self.activation)
...
class NonLinearityLayerForConv1d(Layer):
    ...
    def __repr__(self):
        return "{}: shape={} activation={} rng={}".format(self.__class__.__name__, self.b.shape.eval(), self.activation,
                                                          self.rng)
...
class ConvolutionLayer(Layer):
    ...
    def __init__(self, rng, filter_shape, input_shape=None, W=None):
        ...
        if W is None:
            ...
            W_data = numpy.asarray(rng.uniform(low=-W_bound, high=W_bound, size=filter_shape),
    ...
    def __repr__(self):
        return "{}: filter_shape={}; input_shape={}".format(self.__class__.__name__, self.W.shape.eval(),
                                                            self.input_shape)
...
class FeatureClickModelLayer(Layer):
    """Multi-class Logistic Regression
    """
    def __init__(self, n_in, n_out, max_q_id, max_u_id, click_model_index, dim=1, W=None, b=None, W_feats=None, L_feats=None, DEFAULT_REL=0.25, L=0.5):
        super(FeatureClickModelLayer, self).__init__()
        self.rel_const_weight = 2 * L
        self.rel_model_weight = L
        self.max_q_id = max_q_id
        self.max_u_id = max_u_id
        self.click_model_index = click_model_index
        self.init_dim = 2
        if self.click_model_index == 4:
            self.init_dim = 3
        if not W:
            W = build_shared_zeros((2, n_in, dim), 'W_softmax')
        if not b:
            b = build_shared_zeros((2, dim,), 'b_softmax')
        if not W_feats:
            W_feats = build_shared_zeros((2, self.init_dim * dim, dim), 'W_feats')
        if not L_feats:
            L_feats = build_shared_nums((2, dim,), 0.5, 'L_feats')
        self.W_feats = W_feats
        self.L_feats = L_feats
        self.W = W
        self.b = b
        self.weights = [self.W, self.W_feats]
        self.biases = [self.b, self.L_feats]
        self.dim = dim
    def __repr__(self):
    ...
    def output_func(self, input):
        ...
        id_info = session_info[:, : 1 + 1 * self.dim]

        #UBM
        #e_info = session_info[:, 1 + 1 * self.dim: 1 + 2 * self.dim]
        #r_info = session_info[:, 1 + 2 * self.dim: 1 + 3 * self.dim]
        #c_info = session_info[:, 1 + 3 * self.dim: 1 + 4 * self.dim]
        #add_feature = session_info[:, 1 + 2 * self.dim: 1 + 4 * self.dim]

        #PSCM
        #e_info = session_info[:, 1 + 4 * self.dim: 1 + 5 * self.dim]
        #r_info = session_info[:, 1 + 5 * self.dim: 1 + 6 * self.dim]
        #c_info = session_info[:, 1 + 6 * self.dim: 1 + 7 * self.dim]
        #add_feature = session_info[:, 1 + 5 * self.dim: 1 + 7 * self.dim]

        e_info = session_info[:, 1 + (self.click_model_index + 0) * self.dim: 1 + (self.click_model_index + 1) * self.dim]
        r_info = session_info[:, 1 + (self.click_model_index + 1) * self.dim: 1 + (self.click_model_index + 2) * self.dim]
        c_info = session_info[:, 1 + (self.click_model_index + 2) * self.dim: 1 + (self.click_model_index + 3) * self.dim]

        begin_index = 1
        if self.click_model_index == 4:
            begin_index = 0
        add_feature = session_info[:, 1 + (self.click_model_index + begin_index) * self.dim: 1 + (self.click_model_index + 3) * self.dim]

        exam_w_dot = T.dot(features, self.W[0])
        exam_f_dot = T.dot(add_feature, self.W_feats[0])

        rel_w_dot = T.dot(features, self.W[1])
        rel_f_dot = T.dot(add_feature, self.W_feats[1])

        exam = 1 / (1 + T.exp(-self.L_feats[0] * exam_w_dot - (1 - self.L_feats[0]) * exam_f_dot - self.b[0]))
        rel = 1 / (1 + T.exp(-self.L_feats[1] * rel_w_dot - (1 - self.L_feats[1]) * rel_f_dot - self.b[1]))
        p_1 = exam * rel
        ...
        #pad inner outputs
        #print "!!!!!!!!L\t" + str(self.L_feats[0].type.broadcastable)
        #print "!!!!!!!!exam\t" + str(exam.type.broadcastable)

        cal = exam > 0.5
        tmp0 = self.L_feats[0] * (1.0 + cal) / (1.0 + cal)
        tmp1 = self.L_feats[1] * (1.0 + cal) / (1.0 + cal)
        self.p_y_given_x = T.horizontal_stack(id_info, tmp0, tmp1, exam, rel, p_1)
        ...
        self.exam_model_loss = T.pow(exam - e_info, 2)
        #self.const_model_loss = (T.abs_(tmp0) + 1) * (tmp0 < 0) + (T.abs_(tmp1) + 1) * (tmp1 < 0) + (T.abs_(tmp0) + 1) * (tmp0 > 1) + (T.abs_(tmp1) + 1) * (tmp1 > 1)
        return self.y_pred
    ...
    def negative_log_likelihood(self, y):
        ...
        p_1 = self.p_y_given_x[:, -1 * self.dim:]
        loss = -y * T.log(p_1) - (1-y) * T.log(1-p_1)
        return T.mean(loss) + self.rel_model_weight * T.mean(self.rel_model_loss) + self.rel_model_weight * T.mean(self.exam_model_loss)
        #return T.mean(loss) + self.rel_model_weight * T.mean(self.rel_model_loss)
        #return T.mean(loss)
    ...
    def negative_log_likelihood_sum(self, y):
        ...
        p_1 = self.p_y_given_x[:, -1 * self.dim:]
        loss = -y * T.log(p_1) - (1-y) * T.log(1-p_1)
        return T.sum(loss) + self.rel_model_weight * T.sum(self.rel_model_loss) + self.rel_model_weight * T.sum(self.exam_model_loss)
        #return T.sum(loss) + self.rel_model_weight * T.sum(self.rel_model_loss)
        #return T.sum(loss)
...
class SimpleClickModelLayer(Layer):
    """Multi-class Logistic Regression
    """

    def __init__(self, n_in, n_out, max_q_id, max_u_id, dim=1, W=None, b=None, R_W=None, R_b=None, DEFAULT_REL=0.25, L=1.0):
        self.rel_const_weight = L
        self.rel_model_weight = L
        self.max_q_id = max_q_id
        self.max_u_id = max_u_id
        if not R_W:
            #R = build_shared_nums((max_q_id, max_u_id), DEFAULT_REL, 'Relevance')
            R_W = build_shared_zeros((n_in, dim), 'W_softmax')
            R_b = build_shared_zeros((dim,), 'b_softmax')
            first = 0
        else:
            first = 1
        if not W:
    ...
    def negative_log_likelihood(self, y):
        #return -T.mean(T.log(self.p_y_given_x)[T.arange(y.shape[0]), y])
        p_1 = self.p_y_given_x[:, self.dim: 2* self.dim]
        loss = -y * T.log(p_1) - (1-y) * T.log(1-p_1)
        return T.mean(loss) + self.rel_model_weight * T.mean(self.rel_model_loss)
    ...
    def negative_log_likelihood_sum(self, y):
        ...
        p_1 = self.p_y_given_x[:, self.dim: 2* self.dim]
        loss = -y * T.log(p_1) - (1-y) * T.log(1-p_1)
        return T.sum(loss) + self.rel_model_weight * T.mean(self.rel_model_loss)

    def training_cost(self, y):
        """ Wrapper for standard name """
        #update
        print "!!!!!! update"
        self.R_W = self.R_W - self.R_W + self.W[1]
        self.R_b = self.R_b - self.R_b + self.b[1]
        return self.negative_log_likelihood(y)
...
class MultiDimLogisticRegression(Layer):
    """Multi-class Logistic Regression
    """

    def __init__(self, n_in, n_out, dim=1, W=None, b=None):
        if not W:
            W = build_shared_zeros((n_in, dim), 'W_softmax')
        if not b:
            b = build_shared_zeros((dim,), 'b_softmax')
        print "Init multi dim"
        self.W = W
        self.b = b
        self.weights = [self.W]
    ...
    def output_func(self, input):
        # P(Y|X) = softmax(W.X + b)
        p_1 = 1 / (1 + T.exp(-T.dot(input, self.W) - self.b))
        self.y_pred = p_1 > 0.5
        self.p_y_given_x = T.horizontal_stack(1 - p_1, p_1)
        #self.p_y_given_x = T.nnet.softmax(self._dot(input, self.W) + self.b)
        #self.y_pred = T.argmax(self.p_y_given_x, axis=1)
...
class LogisticRegression2(Layer):
    """Multi-class Logistic Regression
    """
...
class PairwiseLogisticWithFeatsRegression(PairwiseLogisticRegression):
    ...
    def __repr__(self):
        return "{}: W={}, W_feats={}, b={}, lambda".format(self.__class__.__name__, self.W.shape.eval(),
...
class __PairwiseLogisticWithFeatsRegression(PairwiseLogisticRegression):
    ...
    def __repr__(self):
        return "{}: W={}, W_feats={}, b={}".format(self.__class__.__name__, self.W.shape.eval(),
                                                   self.W_feats.shape.eval(), self.b.shape.eval())
...
class PositionPairwiseNoFeatsLayer(Layer):
    def __init__(self, q_in, a_in, position, activation=T.tanh):
        super(PositionPairwiseNoFeatsLayer, self).__init__()

        W = build_shared_zeros((position, q_in, a_in), 'W_softmax_pairwise')
...
class PositionOnlySimPairwiseNoFeatsLayer(Layer):
    def __init__(self, q_in, a_in, position, activation=T.tanh):
        super(PositionOnlySimPairwiseNoFeatsLayer, self).__init__()

        W = build_shared_zeros((position + position * (position - 1) / 2, q_in, a_in), 'W_softmax_pairwise')
...
class _PairwiseLogisticWithFeatsRegression(PairwiseLogisticRegression):
    ...
    def output_func(self, input):
        ...
        self.p_y_given_x = T.nnet.softmax(
            dot + feats_dot + T.dot(q, self.W_q) + T.dot(a, self.W_a) + self.b.dimshuffle('x', 0))
        self.prob = self.p_y_given_x[:, -1]
        self.y_pred = T.argmax(self.p_y_given_x, axis=1)
        return self.y_pred
